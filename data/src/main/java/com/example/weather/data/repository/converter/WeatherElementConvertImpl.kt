...
open class WeatherElementConvertImpl @Inject constructor(private val dispatchers: DataDispatchers) : WeatherElementConvert {
    ...
    override suspend fun convertEntityToListModel(weatherEntity: List<WeatherWithIcons>): List<WeatherElement> {
        return withContext(dispatchers.default) {
            return@withContext weatherEntity.map { (weather, icons) -> weather.toWeatherElement(icons) }
        }
    }

    private fun WeatherEntity.toWeatherElement(icons: List<WeatherIconEntity>): WeatherElement {
        return WeatherElement(
            date = date ?: 0,
            sunrise = sunrise ?: 0,
            sunset = sunset ?: 0,
            ...
            pressure = pressure ?: 0,
            humidity = humidity ?: 0,
            weather = icons.map { it.toWeather() },
            speed = speed ?: 0.0,
            deg = deg ?: 0,
            gust = gust ?: 0.0,
            clouds = clouds ?: 0,
    ...
    private fun WeatherIconEntity.toWeather(): Weather {
        return Weather(
            id = id ?: 0L,
            main = main ?: "",
            description = description ?: "",
    ...
    private fun TemperatureEntity.toTemperature(): Temperature {
        return Temperature(
            day = day ?: 0.0,
            min = min ?: 0.0,
            max = max ?: 0.0,
    ...
    private fun FeelsLikeEntity.toFeelsLike(): FeelsLike {
        return FeelsLike(
            day = day ?: 0.0,
            night = night ?: 0.0,
            eve = eve ?: 0.0,
